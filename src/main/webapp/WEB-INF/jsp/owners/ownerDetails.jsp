						...
						<c:forEach var="visit" items="${pet.visits}">
							<tr>
								...
								<td><spring:url
									...
									</spring:url> <a href="${fn:escapeXml(visitDeleteUrl)}">Delete visit</a></td>
							</tr>
						</c:forEach>
						<thead>
							<tr align="right">
						...
						<c:forEach var="hotel" items="${pet.hotels}">
							<tr>
								...
								<td><petclinic:localDate date="${hotel.startDate}"
										pattern="yyyy-MM-dd" /></td>
								<td><petclinic:localDate date="${hotel.finishDate}"
										pattern="yyyy-MM-dd" /></td>
								<td><spring:url
										value="/owners/{ownerId}/pets/{petId}/hotels/{hotelId}/delete"
										var="hotelDeleteUrl">
									...
									</spring:url> <a href="${fn:escapeXml(hotelDeleteUrl)}">Delete hotel
										book</a></td>
							</tr>
						</c:forEach>
						<tr>
							...
							<td><spring:url
									value="/owners/{ownerId}/pets/{petId}/delete"
									var="deletePetUrl">
									<spring:param name="ownerId" value="${owner.id}" />
									<spring:param name="petId" value="${pet.id}" />
								</spring:url> <a href="${fn:escapeXml(deletePetUrl)}">Delete Pet</a></td>
							<td><spring:url
							...
							<c:if test="${pet.hotels.size() < 2}">
								<td><spring:url
										value="/owners/{ownerId}/pets/{petId}/hotels/new"
										var="hotelUrl">
										<spring:param name="ownerId" value="${owner.id}" />
										<spring:param name="petId" value="${pet.id}" />
									</spring:url> <a href="${fn:escapeXml(hotelUrl)}">Add Hotel Book</a></td>
							</c:if>
						</tr>
					</table> <c:if test="${pet.hotels.size() >= 2}">
						<spring:message
							text="You can not book more for this pet, for that delete one" ></spring:message>
					</c:if>
				</td>
			</tr>
